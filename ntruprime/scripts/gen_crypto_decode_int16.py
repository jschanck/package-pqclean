...
def inner(indent,inpos,m0,m1,bytes,outpos0,outpos1):
  ...
  if bytes == 0:
    ...
    if t == 0:
      stanza += indent + 'a1 = (int16) ((ri<<8)+s0-a0);\n'
    elif t == 8:
      stanza += indent + 'a1 = ri+((s0-a0)>>8);\n'
    elif t < 8:
      stanza += indent + 'a1 = (int16) ((ri<<%d)+((s0-a0)>>%d));\n' % (8-t,t)
    else:
    ...
    assert bytes == 2
    if t == 0:
      stanza += indent + 'a1 = (int16) ((s1<<8)+s0-a0);\n'
    elif t == 8:
      stanza += indent + 'a1 = (int16) ((ri<<8)+s1+((s0-a0)>>8));\n'
    elif t < 8:
      stanza += indent + 'a1 = (int16) ((ri<<%d)+(s1<<%d)+((s0-a0)>>%d));\n' % (16-t,8-t,t)
    else:
      stanza += indent + 'a1 = (int16) (((((int32)ri)<<16)+(s1<<8)+s0-a0)>>%d);\n' % t
