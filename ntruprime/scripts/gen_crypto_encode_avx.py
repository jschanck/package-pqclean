...
def printloop(looplen,reading,todo,m0,bytes):
  ...
  if looplen == 1:
    if reading == 'R0':
        print('  r0 = (uint16) (%s);' % access(reading,0))
        print('  r1 = (uint16) (%s);' % access(reading,1))
    else:
        print('  r0 = %s;' % access(reading,0))
        print('  r1 = %s;' % access(reading,1))
    print('  r2 = r0+r1*(uint32)%d;' % m0)
    for j in range(bytes):
      print('  *out++ = (unsigned char) r2; r2 >>= 8;')
    print('  %s[0] = (uint16) r2;' % todo)
    return
  ...
  if looplen >= 12 and bytes == 1:
    ...
    print('    s0 = (uint32) _mm256_extract_epi32(x,4);')
    print('    *out++ = (unsigned char) s0; s0 >>= 8;')
    print('    *out++ = (unsigned char) s0; s0 >>= 8;')
    print('    *out++ = (unsigned char) s0; s0 >>= 8;')
    print('    *out++ = (unsigned char) s0;')
    print('    s0 = (uint32) _mm256_extract_epi32(x,6);')
    print('    *out++ = (unsigned char) s0; s0 >>= 8;')
    print('    *out++ = (unsigned char) s0; s0 >>= 8;')
    print('    *out++ = (unsigned char) s0; s0 >>= 8;')
    print('    *out++ = (unsigned char) s0;')
  ...
  if reading == 'R0':
      print('    r0 = (uint16) (%s);' % access(reading,'2*i'))
      print('    r1 = (uint16) (%s);' % access(reading,'2*i+1'))
  else:
...
while Mlen > 1:
  ...
  if Mlen&1:
    ...
    if reading == 'R0':
        print('  %s[%d] = (uint16) (%s);' % (todo,looplen,r0))
    else:
    ...
    if bytes1 == bytes0:
      ...
      printloop(looplen,reading,todo,m0,bytes0)
      if reading == 'R0':
          print('  r0 = (uint16) (%s);' % access(reading,2*looplen))
          print('  r1 = (uint16) (%s);' % access(reading,2*looplen+1))
      else:
      ...
      for j in range(bytes1):
        print('  *out++ = (unsigned char) r2; r2 >>= 8;')
      print('  %s[%d] = (uint16) r2;' % (todo,looplen))
...
print('  r0 = %s;' % access(reading,0))
while m1 > 1:
  print('  *out++ = (unsigned char) r0;' + (' r0 >>= 8;' if m1 > 256 else ''))
  m1 = (m1+255)>>8
