...
def poke(todo,pos,contents):
  if todo == 'R0' and div3:
    return '%s[%s] = (int16) (3*%s%+d);' % (todo,pos,contents,-offset)
...
def inner(indent,reading,inpos,m0,m1,bytes,outpos0,outpos1):
  ...
  if bytes==2 and t == 0: #dead store elimination
    stanza += indent + 'a0 = %s[%s];\n' % (reading,inpos)
  else:
  ...
  if bytes == 0:
    stanza += indent + 'a1 = (int16) ((a2-a0)>>%d);\n' % t

  elif bytes == 1:
    if t == 0:
      stanza += indent + 'a1 = (int16) ((a2<<8)+s[i]-a0);\n'
    elif t == 8:
      stanza += indent + 'a1 = a2+((s[i]-a0)>>8);\n'
    elif t < 8:
      stanza += indent + 'a1 = (int16) ((a2<<%d)+((s[i]-a0)>>%d));\n' % (8-t,t)
    else:
    ...
    assert bytes == 2
    if t == 0:
      stanza += indent + 'a1 = (int16) ((s[2*i+1]<<8)+s[2*i]-a0);\n'
    elif t == 8:
      stanza += indent + 'a1 = (int16) ((a2<<8)+s[2*i+1]+((s[2*i]-a0)>>8));\n'
    elif t < 8:
      stanza += indent + 'a1 = (int16) ((a2<<%d)+(s[2*i+1]<<%d)+((s[2*i]-a0)>>%d));\n' % (16-t,8-t,t)
    else:
      stanza += indent + 'a1 = (int16) (((((int32)a2)<<16)+(s[2*i+1]<<8)+s[2*i]-a0)>>%d);\n' % t
...
def inner16(indent,reading,inpos,m0,m1,bytes,outpos0,outpos1):
  ...
  if bytes == 2 and t == 0: #dead store elimination
    stanza += indent + 'A0 = _mm256_loadu_si256((__m256i *) &%s[%s]);\n' % (reading,inpos)
  else:
