...
+		tmp256[0] = gf_mul_vect(_mm256_set1_epi16(gate_value), param256[0]);
+		tmp256[1] = gf_mul_vect(_mm256_set1_epi16(gate_value), param256[1]);
+		
 		for (size_t j = 32 ; j < PARAM_G ; ++j) {
-			tmp[j] = gf_mul(gate_value, PARAM_RS_POLY[j]);
+			tmp.arr16[j] = gf_mul(gate_value, PARAM_RS_POLY.arr16[j]);
 		}
